...
// Kiểm tra folder tồn tại. Nếu không thì tạo mới folder theo đường dẫn
        File theDir = new File(PropertiesHelper.getValue("ScreenShotPath"));
        if (!theDir.exists()) {
            theDir.mkdirs();
        }
//Lưu file ảnh với tên cụ thể vào đường dẫn
        try {
            FileHandler.copy(source, new File(PropertiesHelper.getValue("ScreenShotPath") + nameFile + "_" + dateFormat.format(new Date()) + ".png"));
            System.out.println("Capture screen shot " + nameFile + "_" + dateFormat.format(new Date()) + ".png successfully");
        } catch (IOException e) {
            System.out.println("ERROR:Cannot capture screen shot " + nameFile + "_" + dateFormat.format(new Date()) + ".png");
            throw new RuntimeException(e);
